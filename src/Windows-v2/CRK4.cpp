...
{      
       ...
       cm->dprod->Saida(os);
       os<<"Qual vetor queres que sejas mostrado? 1-Pressão 2-Saturação de Óleo 3-K1 4-K2 5-K3 6-K4 7-Todos" << std::endl;
       int op;
       cin>>op;
       os << "Mostrando os resultados numéricos..." << std::endl;
       switch(op)
       {
       case 1: 
       {    
...
void CRK4::SaidaDisco(std::ofstream& fout){
	fout<<"-------------------------------------------------------------------------------------------------"<<'\n';
	fout<<"A seguir os dados provenientes da integração da EDO de Muskat: "<<'\n';
	fout<<"-------------------------------------------------------------------------------------------------"<<'\n';     
...
void CRK4::Solver(){
     std::cout<<"Resolvendo a Equação Diferencial Ordinária de Muskat: " << '\n';
     for (int s=0; s<ite; s++)
     {   i++;
         CalcK();
         YJ1();
     }
     std::cout<<"Cálculo Terminado!" << '\n';
}
void CRK4::Grafico(){
	std::cout<<"Deseja ver o gráfico da Saturação de Óleo versus Pressão do Reservatório? 1-SIM 2-NÃO"<<'\n';
	int op; std::cin>>op; std::cin.get();
	if (op==1){
	Gnuplot::set_GNUPlotPath("C:/PROGRA~1/gnuplot/bin");
	CGnuplot grafico(xj,yj,"Saturação de Óleo contra Pressão do Reservatório","linespoints","Pressão","Saturação de Óleo"); 
	system("Pause");
	std::cout<<"Deseja salvar o gráficox? 1-SIM 2-NAO"<<'\n';
