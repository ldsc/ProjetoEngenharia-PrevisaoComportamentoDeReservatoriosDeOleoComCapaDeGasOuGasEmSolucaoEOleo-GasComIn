...
class CSimuladorCGGS{
	...
	CSimuladorCGGS(std::string nome){
		...
		var_ctrl=0;
		do{
			fin>>auxvar; p.push_back(auxvar); 
			fin>>auxvar; bo.push_back(auxvar); 
			fin>>auxvar; invbg.push_back(auxvar); 
			fin>>auxvar; rs.push_back(auxvar); 
			fin>>auxvar; mio_mig.push_back(auxvar);
			fin>>auxvar; if(auxvar!=0){dbodp.push_back(auxvar);}
			fin>>auxvar; if(auxvar!=0){dinvbgdp.push_back(auxvar);}
			fin>>auxvar; if(auxvar!=0){drsdp.push_back(auxvar);}
			fin.ignore(5000,'\n');
			var_ctrl++;
		}while(!fin.eof());
